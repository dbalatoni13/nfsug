...
def process_file(filename, outdirectory):
    ...
    with open(filename, "rb") as f:
        ...
        with open(os.path.join(outdirectory, "debug_splits.txt"), "w") as outfile:
            ...
            with io.BytesIO(lineSection.data()) as lineData:
                ...
                srcfile_addresses: Dict[str, List[int]] = dict()
                ...
                for srcinfoChunk in srcinfoChunks:
                    ...
                    for idx, entry in enumerate(srcinfoChunk.entries):
                        ...
                        if sourceFile not in srcfile_addresses:
                            srcfile_addresses[sourceFile] = []
                        ...
                        while True:
                            ...
                            srcfile_addresses[sourceFile].append(
                                lineChunk.cuStartAddress + lineEntry.addrDelta
                            )
                ...
                for src_file, addresses in srcfile_addresses.items():
                    start, end = min(addresses), max(
                        filter(lambda n: n < 0x803A41B5, addresses)
                    )
                    outfile.write(
                        f"{format(start, '08X')}-{format(end, '08X')}: {src_file}\n"
                    )
